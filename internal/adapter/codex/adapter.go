...
func (a *Adapter) Apply(ctx context.Context, r *storage.Resolver, plan writepath.WritePlan) (writepath.WriteReport, error) {
	...
	//
	// Skipped Applies do NOT update state — on-disk bytes are unchanged
	// and their fingerprint matches PreFingerprint. DryRun runs have an
	// empty PostFingerprint.SHA256. The !report.Skipped boolean is the
	// primary guard; the SHA256 != empty check protects DryRun specifically.
	//
	...
	// read-modify-write through a state.yaml flock so Codex's two-file
	// plan (auth.json then config.toml) — and E7's cross-tool commit
	// orchestrator — cannot race on the state file (F1 fix, see
	// internal/adapter/stateio).
	if !plan.DryRun && !report.Skipped && report.PostFingerprint.SHA256 != "" {
		if serr := stateio.RecordApplied(r, adapter.ToolCodex, plan.Target, report.PostFingerprint.SHA256, time.Now()); serr != nil {

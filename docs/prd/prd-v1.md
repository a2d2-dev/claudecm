...
- **2026-06-30 → 2026-07-02 — Implementation-time policy choices** (recorded across the E1–E9 sprint; each entry is a decision made during code review or CI hardening that needs traceability but does NOT re-litigate ADR-0001):

  - **2026-06-30 — Codex `config.toml` owned-key allowlist: explicit provider names, not glob templates.** Decision: enumerate `model_providers.openai.*` and `model_providers.anthropic.*` verbatim in the Codex allowlist rather than a `model_providers.<name>.*` glob. Rationale: FR-5 merge-preserve semantics need a closed set of owned keys to reason about; a glob would silently claim ownership of every provider a user ever adds, breaking the "unknown keys stay untouched" invariant. Adding a third provider is a §4.7 amendment, not an implicit escalation. Refs: PR review of E4-S1; §4.7 Codex row.

  - **2026-06-30 — Codex `auth.json`: OAuth token bundle fields enumerated after review.** Decision: freeze the OAuth bundle allowlist as `auth_mode`, `tokens.access_token`, `tokens.refresh_token`, `tokens.id_token`, `tokens.expires_at`, and `OPENAI_API_KEY`. Rationale: E4-S1 originally listed only `OPENAI_API_KEY`; the round-trip fixtures showed Codex writes the OAuth bundle after a login, and importing that file must preserve — not drop — those fields on subsequent switches. Enumerating them keeps the allowlist closed while covering observed real-world state. Refs: PR review of E4-S1; NFR-T1 fixture matrix.

  - **2026-07-01 — Overlay-as-truth on switch confirmed by implementation (NFR-S6).** Decision: on `switch`, every owned key is reset to the incoming profile's overlay value; keys the new profile omits are removed from the target file rather than carried over from the previously-active profile. Rationale: E3-S4 and E4-S4 plan builders had to pick a policy for "profile A defines key X, profile B does not"; both landed on remove-on-omit to match the NFR-S6 wording and the kubecm context-switch mental model. The alternative (merge-with-prior-owned-keys) would produce a nondeterministic result depending on switch order, defeating `explain`. Refs: E3-S4, E4-S4; NFR-S6.

  - **2026-07-01 — Empty file (0-byte AND whitespace-only) treated as `{}` uniformly across Import and Plan.** Decision: JSON, TOML, and YAML parsers all coerce a file whose content is empty or only whitespace into an empty document. Rationale: E3-S3 (Claude Code Import) hit real Claude Code installs where a first-run `settings.json` was a zero-byte file; strict parsers rejected it, breaking `import`. Extending the rule to whitespace-only and to Plan-time reads keeps Import and Switch symmetric. A malformed-but-non-empty file still hits NFR-S1 refuse-on-malformed. Refs: E3-S3, E3-S4 followup.

  - **2026-07-01 — Null owned-key values dropped during Import.** Decision: when Import encounters an owned key whose parsed value is `null` (JSON), missing (TOML), or unset (YAML), the resulting profile overlay does NOT record the key. Rationale: E4-S3 review found that carrying `null` values through `Doc.Set` / `sjson.Set` would emit the literal `null` on the next `switch`, which is not what the source file said. Treating null as absent preserves the merge-preserve invariant. Refs: E4-S3 review; FR-5 merge-preserve semantics.

  - **2026-07-01 — `--home` global flag wired to ALL 10 commands.** Decision: `--home` is a persistent flag on the root cobra command; every subcommand (`add`, `list`, `current`, `switch`, `explain`, `import`, `edit`, `rename`, `delete`, `restore`, `export`, `completion`, `version`) resolves paths against the flag value before falling back to `$HOME`. Rationale: E6-S8/S9/S10 review showed the flag was set on `switch` and `explain` only; sandbox-based e2e tests (E8-S3/S4/S5) and the quickstart's `--home /tmp/claudecm-demo` pattern require universal coverage. Refs: E6-S8/S9/S10 review; NFR-S3.

  - **2026-07-02 — Rollback restores from in-memory pre-Stage bytes; on-disk backup is audit-only.** Decision: the two-phase commit orchestrator (`internal/commit`) snapshots each target file's bytes into memory during Stage; on Commit failure the orchestrator restores from those in-memory bytes via the same atomic write path. The FR-5 timestamped backup file under `~/.claudecm/backups/` is still written pre-write but is not read during rollback. Rationale: E7-S2/S3 review showed that reading the on-disk backup during rollback introduces a concurrent-edit race between the two phases; the in-memory snapshot is race-free and does not depend on the backup filesystem being writable at the moment of failure. The on-disk backup remains the source of truth for `restore` and for the audit trail. Refs: E7-S2/S3 review; FR-16, NFR-C1.

  - **2026-07-02 — `skipped=true` does NOT anchor the LastAppliedPerTool state.** Decision: when the resolver marks a tool as skipped (e.g. `~/.codex/config.toml` absent), the drift-detection state is left un-anchored for that tool rather than pinned to the current on-disk bytes. Rationale: E5-S4 originally anchored skipped tools to their current hash, which caused every subsequent `explain` to report "no drift" even after the user manually created the file. Leaving skipped tools un-anchored means the first real switch anchors them cleanly. Deferred TODO: teach `explain` to distinguish "never anchored" from "anchored + drift" in output. Refs: E5-S4; deferred to a post-v1 story.

  - **2026-07-02 — macOS runner pinned to `macos-14` for the Go 1.21 CI cell.** Decision: the GitHub Actions matrix uses `macos-14` for Go 1.21 and `macos-latest` for Go 1.22+. Rationale: `macos-latest` is now `macos-26` (Tahoe), whose dyld rejects Go 1.21-linked test binaries with a hard load error; pinning the 1.21 cell to `macos-14` restores CI while keeping the newer cells on the current runner. This is a CI-hosting quirk, not a claudecm compatibility statement — Go 1.21 remains the module's stated minimum. Refs: CI hotfix PR #37; `.github/workflows/ci.yml`.

  - **2026-07-02 — WriteReport shape frozen at E7 wave; warning surface uses `sync.Once` stderr emission for v1.** Decision: the `writepath.WriteReport` struct's public fields are frozen for v1; the "warnings" surface (e.g. TOML comment-loss warnings per NFR-S7) is emitted to stderr from the Transform closure guarded by `sync.Once` per file rather than added to `WriteReport`. Rationale: adding `WriteReport.Warnings` would extend the write-path contract, and the E7 orchestrator ships without it; the `sync.Once` path delivers the user-visible warning without contract churn. A structured `Warnings` field is deferred to a post-v1 story. Refs: E7-S2/S3 review; NFR-S7.
